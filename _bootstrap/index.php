...
if (!createObject('modNamespace',array(
    'name' => 'commerce_messagebird',
    'path' => $componentPath.'/core/components/commerce_messagebird/',
    'assets_path' => $componentPath.'/assets/components/commerce_messagebird/',
),'name', false)) {
    echo "Error creating namespace commerce_messagebird.\n";
}

/* Path settings */
if (!createObject('modSystemSetting', array(
    'key' => 'commerce_messagebird.core_path',
    'value' => $componentPath.'/core/components/commerce_messagebird/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_messagebird',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_messagebird.core_path setting.\n";
}

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_messagebird.assets_path',
    'value' => $componentPath.'/assets/components/commerce_messagebird/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_messagebird',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_messagebird.assets_path setting.\n";
}
...
$assetsUrl = "{$requestUri}assets/components/commerce_messagebird/";

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_messagebird.assets_url',
    'value' => $assetsUrl,
    'xtype' => 'textfield',
    'namespace' => 'commerce_messagebird',
    'area' => 'Paths',
    ...
    echo "Error creating commerce_messagebird.assets_url setting.\n";
}
...
foreach ($settings as $key => $opts) {
    ...
    if (!createObject('modSystemSetting', array(
        ...
        echo "Error creating commerce_messagebird.".$key." setting.\n";
    }
}
...
include $componentPath . '/core/components/commerce_messagebird/vendor/autoload.php';

// Grab the path to our namespaced files
$modulePath = $componentPath . '/core/components/commerce_messagebird/src/Modules/';

// Instruct Commerce to load modules from our directory, providing the base namespace and module path twice
$commerce->loadModulesFromDirectory($modulePath, 'modmore\\CommerceMessageBird\\Modules\\', $modulePath);
